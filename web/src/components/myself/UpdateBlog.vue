...
export default {
  ...
  computed:{
   ...
   mounted(){
    ...
    this.intro = this.blogDetail.intro
    this.content = this.blogDetail.content
    this.dirId = this.blogDetail.userDir
    this.typeId = this.blogDetail.typeId
    this.privateId = this.blogDetail.privateId
  },
  data(){
    return {
      title:'',
  ...
  methods:{
    saveBlog(){
      ...
      this.$axios.put("/blog/updateBlog/"+this.blogDetail.blogId,formDate).then(res => {
        console.log('updateBlog',res);
        if (res.data.data) {
          alert('修改成功')
        }
      })
